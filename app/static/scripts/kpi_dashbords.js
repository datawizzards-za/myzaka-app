$(document).ready(function () {
    ...
    function graphKPIDashboard(recordsJson) {
        ...
        recordsJson.forEach(function (d) {
            ...
            income_len = d['income_expense'].length
            contact_len = d['officials'].length
            //maintenance_len = d['maintenance'].length
            ...
            if (income_len > length) {
                length = income_len
            }
            if (contact_len > length) {
                length = contact_len
            }
            ...
            for (var i = 0; i < length; i++) {
                ...
                if (i < capital_len) {
                    ...
                    tmp['new_assets'] = d['capital'][i]['sum_new_assets']
                    tmp['renewed_assets'] = d['capital'][i]['sum_renewed_assets']
                    tmp['total_assets'] = d['capital'][i]['sum_total_assets']
                } else {
                    tmp['capital_fy'] = 'None'
                    tmp['capital_amount'] = 'None'
                    tmp['capital_name'] = 'None'
                    tmp['new_assets'] = 'None'
                    tmp['renewed_assets'] = 'None'
                    tmp['total_assets'] = 'None'
                }

                if (i < income_len) {
                    tmp['income_fy'] = d['income_expense'][i]['financial_year_end']
                    tmp['income_amount'] = Number(d['income_expense'][i]['amount'])
                    tmp['income_name'] = d['income_expense'][i]['item_label']
                    tmp['income_type'] = d['income_expense'][i]['amount_type']
                } else {
                    tmp['income_fy'] = 'None'
                    tmp['income_amount'] = 'None'
                    tmp['income_name'] = 'None'
                    tmp['income_type'] = 'None'
                }

                if (i < contact_len) {
                    tmp['title'] = d['officials'][i]['title']
                    tmp['name'] = d['officials'][i]['name']
                    tmp['role'] = d['officials'][i]['role']
                    tmp['email_address'] = d['officials'][i]['email_address']
                    tmp['phone_number'] = d['officials'][i]['phone_number']
                } else {
                    tmp['title'] = 'None'
                    tmp['name'] = 'None'
                    tmp['role'] = 'None'
                    tmp['email_address'] = 'None'
                    tmp['phone_number'] = 'None'
                }

                tmp['muni_official'] = tmp['title'] + " " + tmp['name'] + ", " + tmp['role']
        ...
        var defectChart = dc.rowChart("#contact-chart");
        defectChart.data(function (group) { return group.top(10); })
            .width(300)
            .height(390)
        ...
        var incomeFYDim = ndx.dimension(function (d) { return d["income_fy"]; });
        var incomeFYGroup = incomeFYDim.group();
        var filteredIncomeGroup = {
            all: function () {
                return incomeFYGroup.top(Infinity).filter(function (d) {
                    return d.key !== 'None';
                });
            }
        }
